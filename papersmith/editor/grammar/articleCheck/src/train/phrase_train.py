...
f = open("../../data/train/output.txt","r")
ftext = open("../../data/train/text.txt","r")
ftree = open("../../data/train/tree.txt","w")
fconll = open("../../data/train/data.conll","w")
tree = ""
ntree = ""
First = True
ID = 0
    ...
    if line=='(ROOT' and not First:
        ...
        for i in range(len(Words)):
            if i+1!=Root and father[i]==0:
                father[i] = Root
        for i in range(len(Words)):
            print >>fconll, i+1, Words[i], words_lower[i], POS[i], '-'#, father[i], link[i]
        for each in edge:
            print >>fconll, each
        print >>fconll
        Words = []
